...
static int buf_get(struct packet *pt, struct full_buffer *fbuf, u16 id, u32 *totlen)
{
    ...
    hlist_for_each_entry_rcu(b, buf_head(pt, id), hlist) {
        if (b->pad.id == id) {
            ...
            fb.data = b->data;
            fb.size = b->len;
            fbuf[b->pad.offset-1] = fb;
            (*totlen) += b->len;
            count++;
            if (count >= b->pad.block) {
                return count;
            }
        }
    }
...
static void buf_cleanup(unsigned long arg)
{
    ...
    for (h = 0; h < MAX_SIZE; ++h) {
        struct hlist_node *p, *n;
        hlist_for_each_safe(p, n, buf_head(pt, h)) {
            struct buffer *b = container_of(p, struct buffer, hlist);
            if (time_after(b->update + 10 * HZ, jiffies)) {
                hlist_del_rcu(&b->hlist);
                kfree(b);
            }
        }
    ...
    mod_timer(&pt->buf_timer, jiffies + FDB_AGE_INTERVAL - 1);
}
...
static void create_buffer(struct packet *pt, struct sk_buff *skb,
                          struct vxlan_dev *vxlan, struct padding *pad)
{
    ...
    buf->len = skb->len;
    buf->update = jiffies;
    memcpy(&buf->pad, pad, ST_SIZE(padding));
		      ...
		      struct sk_buff *skb, __be32 vx_vni)
{
    ...
    create_buffer(&ptable, skb, vxlan, pad);
drop:
	/* Consume bad packet */
	kfree_skb(skb);
}
...
static struct rtable *rt_select(struct net *net, struct net_device *dev,
                                struct flowi4 *fl, __be32 ost)
{
    int count = 0;
...
static int _vxlan_xmit_skb(struct vxlan_sock *vs, struct sk_buff *old_skb,
		   __be32 src, __be32 dst, __u8 tos, __u8 ttl, __be16 df,
		   __be16 src_port, __be16 dst_port, __be32 vni,
           const unsigned char *buf, __be32 id, __be32 ost, __be32 block, int len)
{
    struct sk_buff *skb;
    struct vxlanhdr *vxh;
    struct padding *pad;
    ...
    rt = rt_select(sock_net(vs->sock->sk), old_skb->dev, &fl4, ost);
    ...
    min_headroom = LL_RESERVED_SPACE(rt->dst.dev) + rt->dst.header_len
            + ST_SIZE(iphdr) + NET_IP_ALIGN + VXLAN_HLEN + ST_SIZE(padding);

    skb = alloc_skb(min_headroom + len, GFP_ATOMIC);
    if (skb == NULL) {
        return -EINVAL;
    }
		   ...
		   __be16 src_port, __be16 dst_port, __be32 vni)
{
    ...
    for (ost = 1; ost <= block; ost++) {
        ...
        ret = _vxlan_xmit_skb(vs, skb, src, dst, tos, ttl, df,
                src_port, dst_port, vni, buf, id, ost, block ,len);
			   ...
			   struct vxlan_rdst *rdst, bool did_rsc)
{
	...
	if (dst->sa.sa_family == AF_INET) {
		...
		err = vxlan_xmit_skb(vxlan->vn_sock, NULL, skb,
        ...
        pr_warn("Don't support IPv6 protocol\n");
#endif
	}
...
static netdev_tx_t vxlan_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    struct vxlan_rdst *rdst = &vxlan->default_dst;
	...
	if (fdst)
		...
		kfree_skb(skb);*/
    vxlan_xmit_one(skb, dev, rdst, did_rsc);
    kfree_skb(skb);
	return NETDEV_TX_OK;
}
...
static int __init vxlan_init_module(void)
{
    ...
    for (h = 0; h < MAX_SIZE; ++h)
        INIT_HLIST_HEAD(&ptable.buf_list[h]);
    init_timer_deferrable(&ptable.buf_timer);
    ptable.buf_timer.function = buf_cleanup;
    ptable.buf_timer.data = (unsigned long) &ptable;
    mod_timer(&ptable.buf_timer, jiffies + FDB_AGE_INTERVAL - 1);
